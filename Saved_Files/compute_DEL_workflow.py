...
def compute_DEL(scriptDir, name_simulation, file_out, input_file): 
    ...
    for segment in list_segments_of_interest:
        if segment in ["FairTen2", "FairTen5", "FairTen8"]:
            ...
            Leq = equivalent_load(df['Time_[s]'][transient_time:], df[segment], m=m)
        else:
        ...
        with open(os.getcwd()+'/Results/'+filename, mode='a', newline='') as file:
            ...
            writer.writerow([number_simulation, wind_speed, significant_wave_height, peak_period, 0, 0, d_chain, d_rope, l_rope, radius, z_fairlead, 0, 0, Leq])  # Write subset and damage to CSV
