...
def analyze_simulation_results(file_settings, sim_folder, current_folder, seeds_folders, input_file, main_file_name):
    ...
    for seed in seeds_folders:
        ...
        fastout_filename_md = os.path.join(current_folder + '/' + sim_folder + '/' + seed + '/' + main_file_name, main_file_name + '.MD.out')
        fastout_filename_out = os.path.join(current_folder + '/' + sim_folder + '/' + seed + '/' + main_file_name, main_file_name + '.out')
        fastin_filename_SeaState = os.path.join(current_folder + '/' + sim_folder + '/' + seed + '/' + main_file_name, 'IEA-15-240-RWT-Nautilus_SeaState_' + main_file_name + '.dat')
        fastin_filename_Inflow = os.path.join(current_folder + '/' + sim_folder + '/' + seed + '/' + main_file_name, 'IEA-15-240-RWT/IEA-15-240-RWT_InflowFile_' + main_file_name + '.dat')

        # Read data
        df_out = FASTOutputFile(fastout_filename_out).toDataFrame()
        SeaState = FASTInputFile(fastin_filename_SeaState)
        Inflow = FASTInputFile(fastin_filename_Inflow)
        
        # === Environmental conditions ===
        wave_direction=SeaState['WaveDir']
        current_speed=SeaState['CurrNSV0']
        current_direction=SeaState['CurrNSDir']
        wind_direction=Inflow['PropagationDir']
        wave_seed = SeaState['WaveSeed(1)']
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        print("Usage: python PostProcessing.py <SIM_FOLDER>")
        sys.exit(1)

    sim_folder = sys.argv[1]
    sim_name = sys.argv[2]
    current_folder = os.getcwd() 
    seeds_folders = list_folders(current_folder + '/' + sim_folder)
